...
function rewriteM3u8(body, url) {
  ...
  // Change this to your PHP proxy URL
  const proxyUrl = 'http://cdn.cinesl.top/proxy.php';
  
  return body.split('\n').map(line => {
    const t = line.trim();
    if (!t || t.startsWith('#')) return line;
    const abs = t.startsWith('http') ? t : t.startsWith('/') ? origin + t : baseDir + t;
    // Rewrite all media files to go through external PHP proxy
    return proxyUrl + '?url=' + encodeURIComponent(abs);
  }).join('\n');
}
